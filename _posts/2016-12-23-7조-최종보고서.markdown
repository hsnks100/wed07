...
택배함의 물품 존재 여부를 감지해준다.  
센서자체의 가변저항을 조절하여 감지 거리를 택배함의 내부에 맞게 조정.  


![](../images/final/motor.png)

택배함의 문을 여닫는데 사용된다.  
LCD입력에 따라 상태변화에 맞게 문을 열고 닫아준다. 


블루투스 모듈
기기의 상태를 스마트폰으로 전송하기 위해서 쓰인다. 택배 기사가 문을 열고 닫고 하는 정보나, 비밀번호가 다시 세팅 될 때의 정보를 전송한다.


# 프로젝트 시나리오 


사용자가 택배 함 사용을 위해 원하는 비밀번호를 설정 해준다.   
택배기사는 택배 함 사용을 위해 “택배기사” 버튼을 눌려 자신의 스마트폰을 통해 일회용 비밀번호를 받는다.  
사용자 및 택배기사가 비밀번호 입력 성공 시 택배 함이 열리게 되고  즉시 사용자의 스마트폰으로 “문 열림“ 알림이 온다.  
택배기사가 물건을 넣게 되면 적외선센서가 인식하여 사용자의 스마트폰으로 택배물의 존재를 알린다.  
택배함의 택배 물 존재여부가 변경되면 LCD에 택배 물 상태가 변경된다.  
...
추가적으로 LCD 에 부산대 로고를 생기는 방법을 생각을 했는데, 하드웨어 메모리 특성상 140kb 를 넣기가 좀 어려웠다. 
