...
System 내부/외부 클럭을 조절하여 원하는 클럭을 생성하고 이를 이용하여 UART(비동기식) 통신을 할 수 있었다. eclipse 를 통해 보낸 코드를 terminal 이라는 새로운 실행 프로그램으로 확인할 수 있었다. 
그리고 오실로스코프로 클럭을 확인하고 보낸 문자에 해당하는 ASCII 값을 binary 형태의 파형으로 볼 수 있어 쉽게 확인할 수 있었다. 이때 왼쪽이 낮은 비트라는 것을 주의해야 했다. 
