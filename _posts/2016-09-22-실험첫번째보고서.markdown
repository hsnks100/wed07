...
### GPIO memory map
![](../images/experiment01/memory_map.png)

앞으로 두고두고 보게될 memory map..
...
RCC_APB2ENR 에서 PORT 들을 enable 시켜줘야 사용이 가능한데,  위에서 표를 보면 PORTAEN, PORTBEN, PORTCEN, PORTDEN 가 보인다.
...
LED 는 기본적으로 PortD 를 건들임으로써 켤 수 있다.

GPIOD_BSRR, GPIOD_CRL 두개의 레지스터에 값을 넣으면서 구현가능하다..
...
처음 다루는 ARM 보드라서 많이 헷갈렸고 어려웠다. 선발대의 발표에서 조금 정보가 덜 주어지는 발표를 듣게 됐는데 처음엔 조원 모두가 불만이었으나, 문제를 해결하면서 우리가 다루는 플랫폼에 대해서 점점 익숙해져가는 우리 모습을 보았다.
만약 모든 정보가 쌩으로 주어지고 거기에 맞춰서 그냥 따라하기만 했다면 해당 정보들이 머리속에 들어가있지 않았을것 같다. 고생한만큼 큰 공부가 되었고, 앞으로 수행할 과제에 대해 스스로 해결하는 힘을 길렀다고 생각한다.

또한 처음이라서 memory 주소를 쌩으로 쓰면서 과제를 했는데, 다음부터는 미리 define 된 bit macro  와 stm32f macro 를 적극 활용해서 readability 를 높이는 방향으로 코딩을 할 것이다. 
